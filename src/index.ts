...
const LobbyRegex = /{"server":"([^"]*)","gametype":"([^"]*)","lobbyname":"([^"]*)"}/;
const LimboRegex = /{"server":"([^"]*)"}/;
    ...
    private static randomMovement(bot: Bot) {        
        setInterval(async () => {
            ...
            if (block && (block.name === 'water' || block.name === 'lava')) {
                ...
                console.log(blueBright(`[MOVEMENT] Moved to ${bot.entity.position.x - x * 10}, ${bot.entity.position.y}, ${bot.entity.position.z - z * 10}`));
                setTimeout(() => {
